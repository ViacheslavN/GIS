#ifndef AGG_SPAN_IMAGE_FILTER_RGB_565_INCLUDED
#define AGG_SPAN_IMAGE_FILTER_RGB_565_INCLUDED
...
inline int R565(const unsigned char *value)
{
  return ((*((int16*)value)) >> 8) & 0xF8;
}
inline int G565(const unsigned char *value)
{
   return  ((*((int16*)value)) >> 3) & 0xFC; 
}
inline int B565(const unsigned char* value)
{
 return ((*((int16*)value)) << 3) & 0xF8;
}

  template<class Source, class Interpolator> 
  class span_image_filter_rgb565_bilinear_clip : 
    public span_image_filter<Source, Interpolator>
  {
  public:
    typedef Source source_type;
    typedef typename source_type::color_type color_type;
    typedef typename OrderType565 order_type;
    typedef Interpolator interpolator_type;
    typedef span_image_filter<source_type, interpolator_type> base_type;
    typedef typename color_type::value_type value_type;
    typedef typename color_type::calc_type calc_type;
    ...
    //--------------------------------------------------------------------
    span_image_filter_rgb565_bilinear_clip() {}
    span_image_filter_rgb565_bilinear_clip(source_type& src, 
    ...
    void generate(color_type* span, int x, int y, unsigned len)
    {
      ...
      do
      {
          ...
          x_lr <  maxx && y_lr <  maxy) 
        {
          ...
          //fg[1] += weight * *fg_ptr++;
          //fg[2] += weight * *fg_ptr++;
          fg[0] += weight * R565(fg_ptr);
          fg[1] += weight * G565(fg_ptr);
          fg[2] += weight * B565(fg_ptr);
          fg_ptr += 2;
