...
#ifndef AGG_VERTEX_SEQUENCE_INTEGER_INCLUDED
#define AGG_VERTEX_SEQUENCE_INTEGER_INCLUDED
...
namespace agg
{
  ...
  class vertex_sequence_int : public pod_bvector<T, S>
  {
  public:
  ...
  void vertex_sequence_int<T, S>::add(const T& val)
  {
    if(base_type::size() > 1)
    {
  ...
  //------------------------------------------------------------------------
  template<class T, unsigned S> 
  void vertex_sequence_int<T, S>::modify_last(const T& val)
  {
    base_type::remove_last();
    add(val);
  }
  ...
  //------------------------------------------------------------------------
  template<class T, unsigned S> 
  void vertex_sequence_int<T, S>::close(bool closed)
  {
  ...
  struct vertex_dist_int
  {
    ...
    bool operator () (const vertex_dist_int& val)
    {
      bool ret = (dist = /*sqrt_int_int(int64(x - val.x) * (x - val.x) + int64(y - val.y) * (y - val.y))*/ calc_distance_int(x, y, val.x, val.y)) > poly_subpixel_scale;
      if(!ret) 
      {
        dist = 0xffffffffffffffffLL - 1;//MD (use LL for long long numbers)
      }
      return ret;
    }
  };
  ...
  struct vertex_dist_cmd_int : public vertex_dist_int
  {
    unsigned cmd;

    vertex_dist_cmd_int() {}
    vertex_dist_cmd_int(int x_, int y_, unsigned cmd_) :
    vertex_dist_int(x_, y_),
      cmd(cmd_)
    {
    }
  };
