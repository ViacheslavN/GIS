...
namespace agg
{
  ...
  //------------------------------------------------------------------------
  vcgen_stroke_int::vcgen_stroke_int() :
m_stroker(),
m_src_vertices(),
m_out_vertices(),
m_shorten(0.0),
...
//------------------------------------------------------------------------
void vcgen_stroke_int::remove_all()
{
...
//------------------------------------------------------------------------
void vcgen_stroke_int::add_vertex(int x, int y, unsigned cmd)
{
  m_status = initial;
  if(is_move_to(cmd))
  {
...
//------------------------------------------------------------------------
void vcgen_stroke_int::rewind(unsigned)
{
  if(m_status == initial)
  {
...
//------------------------------------------------------------------------
unsigned vcgen_stroke_int::vertex(int* x, int* y)
{
  ...
  while(!is_stop(cmd))
  {
    ...
    case out_vertices:
      ...
      {
        const point_i& c = m_out_vertices[m_out_vertex++];
        *x = c.x;
        *y = c.y;
        return cmd;
      }
