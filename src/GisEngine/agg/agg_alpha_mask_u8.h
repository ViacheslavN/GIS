...
namespace agg
{
    ...
    class alpha_plane_u8
    {
    public:
      typedef int8u cover_type;
      typedef alpha_plane_u8 self_type;
      enum cover_scale_e
      { 
        cover_shift = 8,
        cover_none  = 0,
      ...
      alpha_plane_u8() : m_rbuf(0) {}
      explicit alpha_plane_u8(rendering_buffer& rbuf) : m_rbuf(&rbuf), polyClip_(0) {}
    ...
    public:
      ...
      cover_type pixel(int x, int y)
      {
        return *(m_rbuf->row_ptr(y) + x);
      }
      void copy_pixel(int x, int y, cover_type cover)
      {
        if(!polyClip_ ||  polyClip_->isInside(x, y))
        {
          *(m_rbuf->row_ptr(y) + x) = cover;
        }
      }
      cover_type combine_pixel(int x, int y, cover_type cover)
      {
        //cover_type d = pixel(x, y);
        //return (d - ((int(d) * cover + 1) >> 8)) + cover;
        //return (cover_type)((cover_full + cover * pixel(x, y)) >> cover_shift);
        cover_type ret;
        agg::blender_gray8::blend_pix(&ret, pixel(x, y), cover);
        return ret;
      }
      void blend_pixel(int x, int y, cover_type cover)
      {
        //copy_pixel(x, y, combine_pixel(x, y, cover));
        if(!polyClip_ ||  polyClip_->isInside(x, y))
        {
          cover_type* d = m_rbuf->row_ptr(y) + x;
          *d = ((cover + (*d)) - ((cover * (*d) + cover_full) >> cover_shift));
        }
      }
      void copy_hline(int x, int y, unsigned len, cover_type cover)
      {
        if(!polyClip_)
          ::memset(m_rbuf->row_ptr(y) + x, cover, len);
        else
        {
          int8u* dst = m_rbuf->row_ptr(y);
          do
          {
            if(polyClip_->isInside(x, y))
              *dst = cover;
            dst++;
          } while(--len && ++x < (int)m_rbuf->width());
        }
      }
      void copy_vline(int x, int y, unsigned len, cover_type cover)
      {
        int8u* dst = m_rbuf->row_ptr(y);
        do
      ...
      void copy_hspan(int x, int y, unsigned len, const cover_type* covers)
      {
         if(!polyClip_)
          memcpy(m_rbuf->row_ptr(y) + x, covers, len);
        else
        {
          int8u* dst = m_rbuf->row_ptr(y);
          do
      ...
      void blend_hspan(int x, int y, unsigned len, const cover_type* covers)
      {
        int8u* dst = m_rbuf->row_ptr(y) + x;
        do
        {
          if(!polyClip_ || polyClip_->isInside(x, y))
          *dst = (cover_type)((cover_full + (*covers) * (*dst)) >> cover_shift);
          dst++;
          covers++;
          x++;
        }
      ...
      void copy_vspan(int x, int y, unsigned len, const cover_type* covers)
      {
        int8u* dst = m_rbuf->row_ptr(y) + x;
        do
        {
      ...
      void blend_vspan(int x, int y, unsigned len, const cover_type* covers)
      {
        int8u* dst = m_rbuf->row_ptr(y) + x;
        do
        {
    ...
    private:
      alpha_plane_u8(const alpha_plane_u8&);
      const alpha_plane_u8& operator = (const alpha_plane_u8&);
      rendering_buffer* m_rbuf;
      polygon_clipper *polyClip_;
    };
