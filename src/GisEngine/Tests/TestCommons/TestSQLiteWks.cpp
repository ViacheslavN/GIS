...
void TestSQLiteWks()
{
	...
	GisEngine::GeoDatabase::IWorkspacePtr pWks  = GisEngine::GeoDatabase::CSQLiteWorkspace::Open( L"ne_10m_urban_areas_landscan.sqlite", L"d:\\db", true, true);
	if(!pWks.get())
	{
		...
		pWks = GisEngine::GeoDatabase::CSQLiteWorkspace::Create( L"ne_10m_urban_areas_landscan.sqlite", L"d:\\db");
	}

	//GisEngine::GeoDatabase::IFeatureClassPtr pSQLFC = pWks->OpenFeatureClass(L"ne_10m_roads_north_america");
	GisEngine::GeoDatabase::IFeatureClassPtr pSQLFC = pWks->OpenFeatureClass(L"ne_10m_urban_areas_landscan");
	if(!pSQLFC.get())
	{
		...
		pSQLFC = pWks->CreateFeatureClass(L"ne_10m_urban_areas_landscan", pShapeFC->GetFields().get(), true/*, pShapeFC->GetOIDFieldName(), 	pShapeFC->GetShapeFieldName()*/);
