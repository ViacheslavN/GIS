...
namespace GisEngine
{
	namespace Display
	{
		...
		void CLineGenerator::CalcCap(const GPoint& pt0, const GPoint& pt1)
		{
			...
			else
			{
				...
				if (m_width_sign > 0)
				{
					...
					for (i = 0; i < n; i++)
					{
						add_vertex(pt0.x + cos(a1) * m_width, pt0.y + sin(a1) * m_width);
						a1 += da;
					}
				}
				else
		...
		void CLineGenerator::CreateLine(std::vector<GPoint>& vecLine)
		{
			
			uint32 nSize = vecLine.size();
			...
			bool bIsLoop = vecLine[0] == vecLine[vecLine.size() - 1];
			...
			while (nStatus != eStop)
			{
				...
				{
				case eCapIn:
					CalcCap(vecLine[0], vecLine[1]);
					nStatus = eInLine;
					m_nCurrPoint = 1;
					break;
				case eCapOut:
					CalcCap(vecLine[vecLine.size() - 1], vecLine[vecLine.size() - 2]);
					nStatus = eOutLine;
					break;
				case eInLine:
					if (bIsLoop)
					{
						if (m_nCurrPoint >= vecLine.size())
						{
							nStatus = eStop;
							break;
						}
					...
					{
						if (m_nCurrPoint >= vecLine.size() - 1)
						{
							nStatus = eCapOut;
							break;
						}
					}
					{
						...
						CalcJoin(vecLine[nPrevIndex], vecLine[m_nCurrPoint], vecLine[nNextIndex]);
						++m_nCurrPoint;
					}
					break;
					case eOutLine:
					{
						...
						uint32 nPrevIndex = (m_nCurrPoint + nSize - 1) % nSize;
						uint32 nNextIndex = (m_nCurrPoint + 1) % nSize;
						CalcJoin(vecLine[nNextIndex], vecLine[m_nCurrPoint], vecLine[nPrevIndex]);
...
/*
			...
			for (size_t i = 0, sz = vecLine.size() * 2; i < sz; i++)
			{
				if (i == nSize)
					bDirect = false;
				...
				size_t nIndex = bDirect ? i : 2 * nSize - i - 1;
				uint32 nPrevIndex = (nIndex + nSize - 1) % nSize;
				uint32 nNextIndex = (nIndex + 1) % nSize;

			 

				if (bDirect)
				{
					CalcJoin(vecLine[nPrevIndex],	vecLine[nIndex],	vecLine[nNextIndex]);
				}
				else
				{
					CalcJoin(vecLine[nNextIndex],	vecLine[nIndex],  vecLine[nPrevIndex]);
		...
		void CLineGenerator::CalcJoin(const GPoint& pt0, const GPoint& pt1, const GPoint& pt2)
		{
			...
			double dx1 = m_width * (pt1.y - pt0.y) / len1;
			double dy1 = m_width * (pt1.x - pt0.x) / len1;
			double dx2 = m_width * (pt2.y - pt1.y) / len2;
			double dy2 = m_width * (pt2.x - pt1.x) / len2;
			...
			else
			{
				...
				case JoinTypeMiterRevert:
				case JoinTypeMiterRound:
					calc_miter(pt0, pt1, pt2, dx1, dy1, dx2, dy2, m_JoinType, m_miter_limit, dbevel);
					break;
