...
namespace embDB
{
	...
	public:
		...
		BPLeafNodeMapSimpleCompressor(uint32 nPageSize, CommonLib::alloc_t *pAlloc = nullptr, TCompressorParams *pParams = nullptr) : m_nCount(0),
		...
		bool IsHaveUnion(BPLeafNodeMapSimpleCompressor &pCompressor) const
		{
		...
		bool IsHaveAlignment(BPLeafNodeMapSimpleCompressor &pCompressor) const
		{
			uint32 nNoCompSize = m_nCount * (sizeof(TKey) + sizeof(TValue));
