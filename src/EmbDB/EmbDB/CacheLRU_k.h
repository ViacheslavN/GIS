#ifndef _EMBEDDED_DATABASE_CACHE_LRU_K_H_
#define _EMBEDDED_DATABASE_CACHE_LRU_K_H_
...
namespace embDB
{
	...
	class TCacheLRU_2K
	{
		...
		{
		public:
			TListNode(const TKey& _key, TObj _obj) : m_pNext(0), m_pPrev(0), obj_(_obj), key_(_key), m_nK(0)
			{
			}
			TListNode *m_pNext;
			TListNode *m_pPrev;
	...
	public:
		...
		TCacheLRU_2K(CommonLib::alloc_t* pAlloc) : 
		m_pAlloc(pAlloc)
			,m_pFirst(0)
			,m_pBack(0)
		{
		...
		TObj* GetElem(const TKey& key, bool bNotMove = false)
		{
			...
			if(el != m_pFirst)
			{
				...
				while(pNextEl)
				{
					if(el->m_nK > pNextEl->m_nK)
						pNextEl = pNextEl->m_pNext;
				}

				TListNode* pNext = el->m_pNext;
				TListNode* pPrev = el->m_pPrev;

				if(pNextEl)
				{

					TListNode* pNextNext = pNextEl->m_pNext;
					if(pNextNext)
					{
						pNextNext->m_pPrev = el;
						el->m_pNext = pNextNext;
					}
				}
