#ifndef _EMBEDDED_DATABASE_CACHE_NODES_PTR_H_
#define _EMBEDDED_DATABASE_CACHE_NODES_PTR_H_
...
namespace embDB
{
	...
	{
	public:
		TCacheNodePtr(const TVal& _val, TObj _obj) : val_(_val), m_pNext(0), m_pPrev(0), obj_(_obj)
		{
		}
		TCacheNodePtr *m_pNext;
		TCacheNodePtr *m_pPrev;
		TVal val_;
		TObj obj_;
	};

	template<class TKey, class TObj>
	class TSimpleCachePtr
	{
	public:
		TSimpleCachePtr(CommonLib::alloc_t* pAlloc) : 
		  m_pAlloc(pAlloc)
			  , m_set(pAlloc)
			  ,m_pFirst(0)
			  ,m_pBack(0)
		  ...
		  struct SCacheElem
		  {
			  ...
			  {}
			  TCacheNodePtr<TKey, TObj> *pListEl;
		  };
	...
	public:
		...
		typedef TCacheNodePtr<TKey, TObj> TCacheEl;
