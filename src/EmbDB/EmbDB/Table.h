...
namespace embDB
{
	...
	class CTable : public IDBTable
	{
		...
		public:
			...
			virtual IField* getField(const CommonLib::CString& sName) const ;
			...
			IDBFieldHandler* getFieldHandler(const CommonLib::CString& name);

			bool delField(IDBFieldHandler *pField, IDBTransactions *Tran = NULL);
			bool delField(const CommonLib::CString& sFieldName, IDBTransactions *Tran = NULL);
	    ...
	    private:
			...
			typedef std::map<CommonLib::CString, IDBIndexHandler*> TIndexByName;
