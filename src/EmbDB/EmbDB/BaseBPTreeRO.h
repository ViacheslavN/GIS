#ifndef _EMBEDDED_DATABASE_B_PLUS_TREE_READ_ONLY_H_
#define _EMBEDDED_DATABASE_B_PLUS_TREE_READ_ONLY_H_
...
namespace embDB
{
	...
	class _TBreeNode = BPTreeNodeRO<_TKey, _TValue, _TLink, _TComp, _Transaction, _TInnerComp, _TLeafComp> >
	class TBaseBPlusTreeRO
	{
	public:
		TBaseBPlusTreeRO(int64 nPageBTreeInfo, IDBTransaction* pTransaction, CommonLib::alloc_t* pAlloc, uint32 nChacheSize, bool bMulti = false) :
		...
		typedef typename TBTreeNode::TInnerObj TInnerObj;
		typedef typename TBTreeNode::TLeafObj TLeafObj;
		typedef BPTreeStatistics<TLink, Transaction, TKey> BPTreeStatisticsInfo;
		typedef TBPTIteratorRO<TKey, TValue,  TLink, TComp, Transaction, TInnerCompressor, TLeafCompressor, TBTreeNode> iterator;
		...
		TBTreeNode* getNode(TLink nAddr, bool bIsRoot = false, bool bNotMove = false, bool bCheckCache = false)
		{
			...
			if(!pBNode)
			{
				...
				if(!pFilePage)
					return NULL;
				pBNode = new TBTreeNode(m_pAlloc, nAddr, m_bMulti);
