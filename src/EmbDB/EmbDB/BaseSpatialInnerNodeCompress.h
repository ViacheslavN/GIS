...
namespace embDB
{
	...
	public:
		...
		void SplitIn(uint32 nBegin, uint32 nEnd, BPSpatialRectInnerCompressor *pCompressor)
		{
		...
		bool IsHaveUnion(BPSpatialRectInnerCompressor *pCompressor) const
		{
		...
		bool IsHaveAlignment(BPSpatialRectInnerCompressor *pCompressor) const
		{
			uint32 nNoCompSize = m_nCount * (sizeof(ZValueType) + sizeof(TLink));
