...
void ImportShapeFile(const wchar_t* pszDBName, const wchar_t* pszShapeFileName, const wchar_t* pszIDObj, const wchar_t *pszPWD = NULL)
{
	...
	CommonLib::bbox bounds;
	{
		...
		embDB::IDatabasePtr pDatabase = embDB::IDatabase::CreateDatabase(embDB::lmConsole);
		pDatabase->create(pszDBName, embDB::eTMSingleTransactions, sDBPath.wstr(), pszPWD);
 
		pDatabase->setLogLevel(11);
		embDB::IConnectionPtr pConn = pDatabase->connect(NULL, pszPWD);
		...
		for (int fieldNum = 0; fieldNum < fieldCount; ++fieldNum)
		{
			...
			if (fp.m_sFieldName == pszIDObj)
			{
				...
				pTable->createIndex(fp.m_sFieldName, ip, pTran.get());
			}

		}

		pTable->createShapeField(sFileName.wstr(), L"", SHPTypeToGeometryType(shapeType, NULL, NULL), bounds, GetGeometryUnits(units), pTran.get(), true, 8192);
		pTran->commit();
...

	{
		...
		embDB::IDatabasePtr pDatabase = embDB::IDatabase::CreateDatabase(embDB::lmConsole);
		pDatabase->open(pszDBName, embDB::eTMSingleTransactions, sDBPath.wstr());
		 
		pDatabase->setLogLevel(11);
		embDB::IConnectionPtr pConnect = pDatabase->connect(NULL, pszPWD);
		...
		pTran->begin();
		{
			...
			embDB::IInsertCursorPtr pInsertCursor = pTran->createInsertCursor(pTable->getName().cwstr());
			embDB::IRowPtr pRow = pInsertCursor->createRow();
			embDB::IFieldsPtr pFields = pTable->getFields();
...
void SearchShapeFile(const wchar_t* pszDBName, const wchar_t* pszTable, const wchar_t* pszSpField, CommonLib::bbox& bbox, const wchar_t* pszOIDField, const wchar_t* pszPWD)
{
	...
	embDB::IDatabasePtr pDatabase = embDB::IDatabase::CreateDatabase(embDB::lmConsole);
	pDatabase->open(pszDBName, embDB::eTMSingleTransactions, sFilePath.wstr());
